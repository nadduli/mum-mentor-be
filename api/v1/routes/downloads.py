...
async def track_download(payload: DownloadTrackRequest, request: Request, db: Session = Depends(get_db)):
    event, error = log_download_event(db, request, payload, user_id=None, session_id=None)
    if error:
        logger.error(f"track_download_error: {error}")
        return fail_response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, message=error)
    return success_response(status_code=status.HTTP_201_CREATED, message="Download tracked", data={
        "id": str(event.id),
...
async def download_stats(
    ...
    data, error = get_download_stats(db, from_dt=from_dt, to_dt=to_dt, group_by=group_by)
    if error:
        status_code = status.HTTP_400_BAD_REQUEST if "group_by" in error.lower() else status.HTTP_500_INTERNAL_SERVER_ERROR
