...
async def create_category(
    ...
    try:
        ...
        logger.error(f"HTTP error creating category {e.status_code}: {str(e)}", exc_info=True)
...
async def get_resources(
    q: Optional[str] = Query(None, min_length=1, description="Search term (Title, Content, or Category)"),
