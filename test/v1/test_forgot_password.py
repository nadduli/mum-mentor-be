...
class TestForgotPassword:
    ...
    def mock_send_email(self):
        ...
        with patch('api.v1.services.forgot_password.send_email', new_callable=AsyncMock) as mock:
            mock.return_value = None
            yield mock

    def test_forgot_password_success_returns_200(self, client, test_user, db_session, mock_send_email):
        """
        ...
        otp_record = db_session.query(UserOTPVerification).filter(
    ...
    def test_forgot_password_inactive_user_returns_403(self, client, test_user, db_session, mock_send_email):
        """
    ...
    def test_forgot_password_email_sending_fails_returns_500(self, client, test_user, mock_send_email):
        """
        ...
        data = response.json()
        assert "detail" in data
        assert data["detail"] == "Failed to send password reset code. Please try again later."
    ...
    def test_forgot_password_case_insensitive_email(self, client, test_user, db_session, mock_send_email):
        """
    ...
    def test_forgot_password_invalidates_old_otps(self, client, test_user, db_session, mock_send_email):
        """
        ...
        active_otps = db_session.query(UserOTPVerification).filter(
            UserOTPVerification.user_id == test_user.id,
            UserOTPVerification.otp_type == "password_reset",
            UserOTPVerification.used == False
        ).all()
    ...
    def test_forgot_password_otp_expiration_time(self, client, test_user, db_session, mock_send_email):
        """
