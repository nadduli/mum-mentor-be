...
def test_create_journal_entry_with_new_category(client, test_user):
    ...
    journal = db.query(Journal).filter(Journal.id == uuid.UUID(data["data"]["journal_entry_id"])).first()
...
def test_create_journal_entry_with_existing_category(client, test_user):
    ...
    db = TestingSessionLocal()
    categories = db.query(JournalCategory).filter(JournalCategory.name == "Health").all()
