...
def test_delete_journal_with_photos_cascade(client, test_user, journal_with_photos):
    ...
    db = TestingSessionLocal()
    photos_before = db.query(JournalPhoto).filter(JournalPhoto.journal_id == journal_with_photos).all()
    ...
    db = TestingSessionLocal()
    deleted_journal = db.query(Journal).filter(Journal.id == journal_with_photos).first()
    photos_after = db.query(JournalPhoto).filter(JournalPhoto.journal_id == journal_with_photos).all()
