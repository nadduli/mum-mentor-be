...
def test_category_lifecycle(client):
    ...
    # 1. Create
    create_resp = client.post("/api/v1/resources/categories", json={"name": "Lifecycle Cat"})
    ...
    # 3. Update
    update_resp = client.patch(f"/api/v1/resources/categories/{cat_id}", json={"name": "Updated Cat"})
...
def test_get_all_categories(client):
    ...
    client.post("/api/v1/resources/categories", json={"name": f"List Cat {uuid.uuid4()}"})
...
def test_create_resource_success(client):
    ...
    # Setup Category
    cat_resp = client.post("/api/v1/resources/categories", json={"name": f"Cloud {uuid.uuid4()}"})
...
def test_resource_update_and_delete(client):
    ...
    # Setup
    cat_resp = client.post("/api/v1/resources/categories", json={"name": f"DevOps {uuid.uuid4()}"})
    cat_id = cat_resp.json()["data"]["id"]

    # Create
    res_resp = client.post("/api/v1/resources/", json={"title": "Old Title", "content": "x", "category_id": cat_id})
    res_id = res_resp.json()["data"]["id"]

    # Update
    update_resp = client.patch(f"/api/v1/resources/{res_id}", json={"title": "New Title", "content": "y"})
    assert update_resp.status_code == 200
...
def test_unified_endpoint_search(client):
    ...
    """
    # Setup Data
    cat = client.post("/api/v1/resources/categories", json={"name": f"SearchCat {uuid.uuid4()}"}).json()["data"]
    client.post("/api/v1/resources/", json={"title": "Python Basics", "content": "Code", "category_id": cat["id"]})
    client.post("/api/v1/resources/", json={"title": "Cooking 101", "content": "Food", "category_id": cat["id"]})
...
def test_unified_endpoint_category_filter(client):
    ...
    """
    # Setup Categories
    cat_a = client.post("/api/v1/resources/categories", json={"name": f"Cat A {uuid.uuid4()}"}).json()["data"]
    cat_b = client.post("/api/v1/resources/categories", json={"name": f"Cat B {uuid.uuid4()}"}).json()["data"]

    # Setup Resources
    client.post("/api/v1/resources/", json={"title": "Res A1", "content": "x", "category_id": cat_a["id"]})
    client.post("/api/v1/resources/", json={"title": "Res A2", "content": "x", "category_id": cat_a["id"]})
    client.post("/api/v1/resources/", json={"title": "Res B1", "content": "x", "category_id": cat_b["id"]})
...
def test_unified_endpoint_search_and_filter_combined(client):
    ...
    """
    cat_tech = client.post("/api/v1/resources/categories", json={"name": f"Tech {uuid.uuid4()}"}).json()["data"]
    cat_news = client.post("/api/v1/resources/categories", json={"name": f"News {uuid.uuid4()}"}).json()["data"]
