...
class TestUserRegistration:
    ...
    def test_duplicate_email_registration_unverified(self, client: TestClient, sample_user_data: dict):
        ...
        # Second registration with same email (unverified) - should succeed and update
    ...
    def test_duplicate_email_registration_verified(self, client: TestClient, db_session, sample_user_data: dict):
        """Test registration with verified email returns 409 Conflict."""
        from api.v1.models.user.user import User
        
        # First registration and verify
        response1 = client.post("/api/v1/auth/register", json=sample_user_data)
        assert response1.status_code == status.HTTP_201_CREATED
        
        # Manually verify the user
        user = User.fetch_unique(db_session, email=sample_user_data["email"])
        user.email_verified = True
        user.update(db_session)
        
        # Attempt second registration with verified email
    ...
    def test_registration_email_case_insensitivity(self, client: TestClient, db_session, sample_user_data: dict):
